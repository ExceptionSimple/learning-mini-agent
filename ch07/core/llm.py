...
class DeepSeekLLM:
    ...
    def _parse_tool_calls(self, raw_tool_calls: list | None) -> list[ToolCall]:
        """把 API 原始 tool_calls（arguments 为 JSON 字符串）解析为 ToolCall 对象。"""
        parsed = []
    ...
    def invoke(self,
        ...
        logger.info('收到响应: id=%s, finish_reason=%s', data['id'], finish_reason)

        # 输出槽位：与 stream 一致，解析为 ToolCall 对象后存实例，供调用方读取
...
#     #     print(delta, end='')
    ...
    # response = chat.invoke(messages=messages, model=os.environ.get("DEEPSEEK_MODEL"))
    # print(response)
